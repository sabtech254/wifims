...
switch ($action) {
    ...
    case 'app':
        ...
        if (!empty(_get('testWa'))) {
            $result = Message::sendWhatsapp(_get('testWa'), 'wifims Test Whatsapp');
            r2(U . "settings/app", 's', 'Test Whatsapp has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testSms'))) {
            $result = Message::sendSMS(_get('testSms'), 'wifims Test SMS');
            r2(U . "settings/app", 's', 'Test SMS has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testEmail'))) {
            Message::sendEmail(_get('testEmail'), 'wifims Test Email', 'wifims Test Email Body');
            r2(U . "settings/app", 's', 'Test Email has been send');
        }
        if (!empty(_get('testTg'))) {
            $result = Message::sendTelegram('wifims Test Telegram');
            ...
            if ($radius_enable) {
                try {
                    ...
                    $ui->assign("error_message", "Radius table not found.<br><br>" .
                        $e->getMessage() .
                        "<br><br>Download <a href=\"https://raw.githubusercontent.com/sabtech254/wifims/Development/install/radius.sql\">here</a> or <a href=\"https://raw.githubusercontent.com/sabtech254/wifims/master/install/radius.sql\">here</a> and import it to database.<br><br>Check config.php for radius connection details");
